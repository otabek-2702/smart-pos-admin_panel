...
function Select(props) {
  ...
  return (
    <div className={cx("control control--select", size === "sm" && "control--sm", error && "is-error", disabled && "is-disabled")}>
      {icon && <Icon name={icon} size={18} />}
      <select value={value} disabled={disabled}
        ...
        {options.map((o) => {
          ...
          return <option key={val} value={val} style={{ color: "var(--text)" }}>{lab}</option>;
        })}
      </select>
...
function Kpi(props) {
  ...
  if (typeof d.value === "string") { display = d.value; }
  else if (isMoney) { display = <CountUp value={d.value} format={Fmt.abbr} />; unit = "UZS"; }
  else { display = <CountUp value={d.value} format={Fmt.num} />; }
  return (
    <div className="kpi">
      ...
      <div className="kpi__foot">
        {d.delta !== null && d.delta !== undefined && <Delta value={d.delta} />}
        {d.sub && <span className="kpi__subtext">{d.sub}</span>}
        {d.deltaLabel && <span className="kpi__subtext">{d.deltaLabel}</span>}
      </div>
    </div>
  );
}
...
function StateFill(props) {
  return (
    <div className={cx("statefill", props.error && "is-error")} style={props.style}>
...
function Pagination(props) {
  ...
  else if (page <= 4) { nums = [1, 2, 3, 4, 5, "…", pages]; }
  else if (page >= pages - 3) { nums = [1, "…", pages - 4, pages - 3, pages - 2, pages - 1, pages]; }
...
Object.assign(window, {
  cx, Button, IconAction, Field, Input, Select, Switch, Segmented, Checkbox,
  Card, Delta, Kpi, KpiMini, Badge, StatusBadge, Skeleton, StateFill,
