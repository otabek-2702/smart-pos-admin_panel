...
/* ---------- Sidebar ambient "today" widget — always-visible live data ---------- */
function SidebarLiveWidget(props) {
  const spark = window.DASH && window.DASH.revenue30 ? window.DASH.revenue30.slice(-12) : [8, 9, 8.5, 10, 9.5, 11, 10.5, 12, 11.5, 13, 12.5, 14];
  const [rev, setRev] = useState(14230000);
  const [orders, setOrders] = useState(142);
  useEffect(() => {
    const iv = setInterval(() => {
      setRev((r) => r + Math.floor(Math.random() * 40000));
      setOrders((o) => o + (Math.random() > 0.6 ? 1 : 0));
    }, 4200);
    return () => clearInterval(iv);
  }, []);
  return (
    <div className="sidewidget" onClick={() => props.onNav && props.onNav("dashboard")} title="Open dashboards">
      <div className="row between" style={{ marginBottom: 8 }}>
        <span className="sidewidget__label"><span className="livedot" /> Today</span>
        <Icon name="chevright" size={14} style={{ color: "var(--text-tertiary)" }} />
      </div>
      <div className="sidewidget__value mono" data-comment-anchor="60ecb97e1c-div-155-7">{Fmt.money(rev)}<span className="sidewidget__unit">UZS</span></div>
      <div className="sidewidget__spark">{window.Sparkline && <Sparkline data={spark} width={196} height={30} color="var(--primary)" dot={false} />}</div>
      <div className="row between" style={{ marginTop: 6 }}>
        <span className="sidewidget__meta">{orders} orders</span>
        <span className="delta is-up" style={{ fontSize: 11, padding: "1px 6px" }}><Icon name="arrowup" size={11} weight={2.2} />12.4%</span>
      </div>
    </div>);
...
function SettingsMenu() {
  const [open, setOpen] = useState(false);
  const [dayStart, setDayStart] = useState(() => { try { return localStorage.getItem("alphapos-daystart") || "06:00"; } catch (e) { return "06:00"; } });
  const [weekStart, setWeekStart] = useState(() => { try { return localStorage.getItem("alphapos-weekstart") || "mon"; } catch (e) { return "mon"; } });
  const ref = useRef(null);
  useEffect(() => { try { localStorage.setItem("alphapos-daystart", dayStart); } catch (e) {} }, [dayStart]);
  useEffect(() => { try { localStorage.setItem("alphapos-weekstart", weekStart); } catch (e) {} }, [weekStart]);
  useEffect(() => {
    if (!open) return;
    const h = (e) => { if (ref.current && !ref.current.contains(e.target)) setOpen(false); };
  ...
  return (
    <div style={{ position: "relative" }} ref={ref}>
      <button className={cx("iconbtn", open && "is-active")} onClick={() => setOpen((o) => !o)} title="Settings"><Icon name="gear" size={18} /></button>
      {open && (
        <div className="card setmenu">
          <div className="setmenu__title">Settings</div>
          <div className="setmenu__row">
            <div className="setmenu__main">
              <div className="setmenu__label">Business day starts at</div>
              <div className="setmenu__hint">Sales after midnight count toward the previous day until this time.</div>
            </div>
            <div style={{ width: 110, flex: "0 0 110px" }}><TimeField value={dayStart} onChange={setDayStart} icon="clock" /></div>
          </div>
          <div className="hr" style={{ margin: "4px 0" }} />
          <div className="setmenu__row">
            <div className="setmenu__main">
              <div className="setmenu__label">Week starts on</div>
              <div className="setmenu__hint">Used in weekly reports and the date picker.</div>
            </div>
            <div style={{ width: 130, flex: "0 0 130px" }}>
              <Select size="sm" value={weekStart} onChange={setWeekStart} options={[{ value: "mon", label: "Monday" }, { value: "sun", label: "Sunday" }]} />
            </div>
          </div>
        </div>
      )}
...
function App() {
  ...
  useEffect(() => {
    if (drawerOpen) document.body.style.overflow = "hidden";else
  ...
  const handleToggleNav = () => {
    if (typeof window !== "undefined" && window.matchMedia("(max-width: 768px)").matches) setDrawerOpen((o) => !o);else
    setCollapsed((c) => !c);
  };
  ...
  const showDate = route === "analytics" || route === "shifts";
