...
function AIChatPage(props) {
  ...
  return (
    <div className={cx("aiwrap", histOpen && "hist-open")}>
      {/* history sidebar */}
        ...
        <div className="aihist__list">
          ...
          {filtered.length === 0 ? <div className="aihist__empty">No chats found</div> : filtered.map((c) => (
            <ChatHistoryItem key={c.id} chat={c} active={c.id === ai.activeId} generating={ai.generating === c.id}
              onClick={() => { ai.selectChat(c.id); setHistOpen(false); }} onDelete={() => ai.deleteChat(c.id)} />
          ))}
        </div>
      </aside>
      <div className="aihist-scrim" onClick={() => setHistOpen(false)} />

      {/* thread */}
      <section className="aithread">
        <div className="aithread__head">
          <div className="row" style={{ gap: 10, minWidth: 0 }}>
            <button className="aihist__toggle" onClick={() => setHistOpen(true)} title="Chat history"><Icon name="menu" size={20} /></button>
