/* ============================================================
   ALPHA POS — motion composables (Vue port of .tmp-design-bundle/app/anim.jsx)

   Resting state (no `html.anim`) is fully visible. Adding `.anim` arms
   the hidden state; `.anim.rin` reveals it. Transitions (NOT keyframes)
   resolve to their END state even in hidden/background tabs, so
 ...
 * 1200ms (matches the bundle's bootstrap behaviour). Call once at app boot.
...
export function armMotion() {
  ...
  // Match anim.jsx: 1200ms delay before reveal. Reveals via transition
  // means content reaches the visible state even if the tab is hidden.
 ...
 * the next tick. Mirrors anim.jsx::replayMotion. Wire to router.afterEach so
...
/**
 * Counts a number from 0 → target with ease-out-cubic over `duration` ms.
 * Mirrors anim.jsx::useCountUp.
 *
 ...
 */
export function useCountUp(target: Ref<number> | (() => number), opts?: { duration?: number }): Ref<number> {
  const duration = opts?.duration ?? 900
  const val = ref(0)
  const prev = { current: 0 }
  const reduced = getPrefersReduced()
  const getter: () => number = typeof target === 'function' ? target as () => number : () => (target as Ref<number>).value
  ...
  function start() {
    ...
    if (cancel())
      return
    if (reduced || typeof t !== 'number' || !Number.isFinite(t) || (typeof document !== 'undefined' && document.hidden)) {
