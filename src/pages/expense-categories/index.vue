...
const emptyTitle = computed(() => t(search.value.trim() ? 'No matching categories' : 'expcat_empty_title'))
const emptySubtitle = computed(() => t(search.value.trim() ? 'expcat_empty_search_hint' : 'expcat_empty_subtitle'))
let listRequestId = 0
...
async function load() {
  ...
  try {
    ...
    if (requestId !== listRequestId || !canView.value)
      return
...
function openCreate() {
  if (!canManage.value || saving.value || deactivating.value)
    return
...
function toggleSource(source: ExpenseSource, enabled: boolean) {
  if (saving.value || !canManage.value || !formOpen.value)
    return
...
function validate() {
  ...
  if (!form.value.allowed_sources.length || form.value.allowed_sources.some(source => !SOURCE_OPTIONS.includes(source)))
...
async function submit() {
  if (saving.value || deactivating.value || !canManage.value || !formOpen.value)
    return
  if (!validate())
    return
...
function askDeactivate(row: ExpenseCategory) {
  if (!canManage.value || saving.value || deactivating.value || !row.is_active)
    return
...
async function doDeactivate() {
  if (!canManage.value || saving.value || deactivating.value || !confirmOpen.value || !confirmRow.value?.is_active)
    return
  ...
  try {
    ...
    deactivateError.value = apiError(error)
    notify(deactivateError.value, 'error')
  }
  finally {
      ...
      <div class="toolbar toolbar--wrap">
        ...
        >
          <Switch
            ...
            :aria-label="t('filter_include_inactive')"
          />
      ...
      >
        ...
        <div class="form-grid">
          ...
          >
            <Input
              v-model="form.code"
              :disabled="!!editing || saving || !canManage"
              :error="!!errors.code"
          ...
          >
            <div class="policy-grid">
              <label class="toggle-card">
                <Switch
                  v-model="form.requires_receipt"
                  :aria-label="t('expense_requires_receipt')"
                  :disabled="saving || !canManage"
                />
              ...
              <label class="toggle-card">
                <Switch
                  v-model="form.requires_description"
                  :aria-label="t('expense_requires_description')"
                  :disabled="saving || !canManage"
                />
              ...
              <label class="toggle-card">
                <Switch
                  ...
                  :aria-label="t('expcat_field_is_active')"
                  :disabled="saving || !canManage"
                />
    ...
    >
      ...
      <template #footer>
        <Button
          ...
          :disabled="deactivating || !canManage"
          @click="doDeactivate"
        >
...
.form-error { margin-inline: 0; overflow-wrap: anywhere; }
