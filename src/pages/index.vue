...
import axios, { hrApi } from '@/plugins/axios'
...
async function loadDashboard() {
  ...
  try {
    ...
    const [oStats, empStats, hrCash, pStats, orders] = await Promise.allSettled([
      axios.get('/orders/stats', { params: { ...dateParams } }),
      hrApi.get('/employees/stats/'),
      hrApi.get('/cash/balance/'),
      axios.get('/products/stats'),
      axios.get('/orders', { params: { page: 1, per_page: 8, ...dateParams } }),
    ])

    if (oStats.status === 'fulfilled')
      orderStats.value = oStats.value.data?.data ?? oStats.value.data
    if (empStats.status === 'fulfilled')
      userStats.value = empStats.value.data?.data ?? empStats.value.data
    if (hrCash.status === 'fulfilled')
      inkassaBalance.value = hrCash.value.data?.data ?? hrCash.value.data
    if (pStats.status === 'fulfilled')
      productStats.value = pStats.value.data?.data ?? pStats.value.data
          ...
          <VCardText class="d-flex align-center gap-3">
            ...
            <div>
              ...
              <div
                v-else
                class="sk-box mb-1"
                style="width:90px;height:22px;border-radius:4px;"
              />
              <div class="text-body-2 text-disabled">
                {{ t('Cash Balance') }}
              </div>
      ...
      >
        <VCard>
          <VCardText class="text-center pa-3">
            <div
              v-if="orderStats"
              class="text-h5 font-weight-bold text-info"
            >
      ...
      >
        <VCard>
          <VCardText class="text-center pa-3">
            ...
            />
            <div class="text-caption text-disabled">
              {{ t('Paid') }} ({{ orderStats?.paid_orders ?? 0 }}/{{ orderStats?.total_orders ?? 0 }})
            </div>
          </VCardText>
        </VCard>
      </VCol>
      ...
      >
        <VCard>
          <VCardText class="text-center pa-3">
            <div
              v-if="productStats"
              class="text-h5 font-weight-bold text-secondary"
            >
          ...
          <VCardText class="pb-2">
                ...
                <div class="d-flex align-center gap-2">
                  ...
                  <div>
                    <div class="text-body-2 font-weight-medium">
                      {{ (c as any).cashier_name }}
                    </div>
                    <div class="text-caption text-disabled">
                      {{ (c as any).order_count }} {{ t('Orders').toLowerCase() }}
                    </div>
                  </div>
                </div>
            ...
            <template v-else-if="loading">
              ...
              >
                ...
                <div
                  class="sk-box"
                  style="width:100px;height:12px;border-radius:4px;"
                />
              </div>
            </template>
            <div
              v-else
              class="text-center text-disabled text-body-2 py-2"
            >
              {{ t('No data') }}
            </div>
          </VCardText>
          ...
          <VCardText class="pb-2">
            ...
            <template v-else-if="loading">
              ...
              >
                ...
                <div
                  class="sk-box"
                  style="width:30px;height:18px;border-radius:8px;"
                />
              </div>
            </template>
            <div
        ...
        <VCard :title="t('Recent Orders')">
          ...
          >
            ...
            >
              ...
              >
                <td class="sk-cell">
                  <div
                    class="sk-box"
                    style="width:35px;height:13px;border-radius:4px;"
                  />
                </td>
                <td class="sk-cell">
                  <div
                    class="sk-box"
                    style="width:70px;height:20px;border-radius:12px;"
                  />
                </td>
                <td class="sk-cell">
                  <div
                ...
                <td class="sk-cell">
                  <div
                    class="sk-box"
                    style="width:55px;height:20px;border-radius:12px;"
                  />
                </td>
                <td class="sk-cell">
                  <div
                ...
                <td class="sk-cell">
                  <div
                    class="sk-box"
                    style="width:20px;height:13px;border-radius:4px;"
                  />
                </td>
                <td class="sk-cell">
                  <div
            ...
            <template #item.order_type="{ item }">
              <VChip
                size="small"
                :color="orderTypeColors[item.raw.order_type] ?? 'secondary'"
                variant="tonal"
              >
                {{ item.raw.order_type }}
              </VChip>
          ...
          <VCardText class="pa-0">
            ...
            <template v-else-if="loading">
              <div
                v-for="n in 5"
                :key="n"
                class="d-flex align-center gap-3 px-4 py-2"
              >
                <div
                  class="sk-box"
                  style="width:28px;height:28px;border-radius:50%;"
            ...
            <div
              v-else
              class="text-center text-disabled py-4 text-body-2"
            >
              {{ t('No data') }}
            </div>
          </VCardText>
