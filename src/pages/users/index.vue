...
const roles = ['ADMIN', 'CASHIER', 'WAITER', 'USER']
...
async function save() {
  ...
  try {
    if (editing.value) {
      // PATCH — only send fields that changed (password only if provided)
      const payload: any = {
          ...
          <VRow>
            ...
            <VCol cols="12">
              <VTextField
                ...
                :label="editing ? t('New Password (leave blank to keep)') : t('Password')"
                type="password"
              />
            </VCol>
          </VRow>
