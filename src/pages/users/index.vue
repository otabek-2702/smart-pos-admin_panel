...
function onRoleChange() {
  // Non-admin email is auto-generated by backend; clear any stale input.
...
async function save() {
  // ADMIN role requires explicit email; other roles get one auto-assigned by BE.
  if (!editing.value && isAdminRole.value && !form.value.email.trim()) {
    notify(t('Email is required for ADMIN'), 'error')
  ...
  try {
    if (editing.value) {
      ...
      // Email only sent for ADMIN — BE auto-generates for other roles.
      // Status is never sent on create — BE hardcodes ACTIVE.
      if (isAdminRole.value && form.value.email)
        payload.email = form.value.email
          ...
          <VRow>
            ...
            >
              <VTextField
                ...
                :label="t('Email')"
                type="email"
                :hint="!editing && isAdminRole ? t('Required for ADMIN role') : ''"
                :persistent-hint="!editing && isAdminRole"
                :required="!editing && isAdminRole"
              />
            </VCol>
            <VCol cols="12">
              <VTextField
